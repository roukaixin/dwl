...
/* enums */
enum {
    CurNormal, CurPressed, CurMove, CurResize
}; /* cursor */
enum {
    XDGShell, LayerShell, X11
};                 /* client types */
enum {
    LyrBg,
    LyrBottom,
...
static void
draw_rect(pixman_image_t *pix, int16_t x, int16_t y, uint16_t w,
...
mapnotify(struct wl_listener *listener, void *data)
{
    ...
    if (client_is_unmanaged(c)) {
        ...
        wlr_scene_node_set_position(&c->scene->node, c->geom.x + (int) borderpx, c->geom.y + (int) borderpx);
...
setfloating(Client *c, int floating)
{
    ...
    wlr_scene_node_reparent(
            &c->scene->node,
            layers[c->isfullscreen || (p && p->isfullscreen) ? LyrFS : c->isfloating ? LyrFloat : LyrTile]
    );
    arrange(c->mon);
    drawbars();
}
